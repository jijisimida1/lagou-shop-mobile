    ...
    <div class="main">
      ...
      <van-cell-group inset>
        ...
        <van-grid column-num="5" :border="false">
          <!-- <van-grid-item badge="0" icon="bill-o" text="待付款" />
          <van-grid-item badge="0" icon="tosend" text="待发货" />
          <van-grid-item badge="0" icon="logistics" text="待收货" />
          <van-grid-item badge="0" icon="comment-o" text="待评价" />
          <van-grid-item badge="0" icon="sign" text="已完成" /> -->
          <van-grid-item
...
// 订单信息展示
const orderNum = computed(() => userData.value?.orderStatusNum)
// 将展示的订单信息设置为配置对象，在模板中遍历绑定
const orderOptions = ref([
  {
    badge: computed(() => orderNum.value?.unpaid_count || ''),
    icon: 'bill-o',
    text: '待付款',
    type: '0'
  },
  {
    badge: computed(() => orderNum.value?.unshipped_count || ''),
    icon: 'tosend',
    text: '待发货',
    type: '1'
  },
  {
    badge: computed(() => orderNum.value?.received_count || ''),
    icon: 'logistics',
    text: '待收货',
    type: '2'
  },
  {
    badge: computed(() => orderNum.value?.evaluated_count || ''),
    icon: 'comment-o',
    text: '待评价',
    type: '3'
  },
  {
    badge: computed(() => orderNum.value?.complete_count || ''),
    icon: 'sign',
    text: '已完成',
    type: '4'
  }
